...
def populate_sensor_collection(db_name, collection_name, base_path, mongo_uri="mongodb://localhost:27017/", batch_size=1):
    """
    ...
    if collection_name == "ambientsensor":
        ...
        sensor_data_generator = create_user_recordings_json(base_path)
    else:
...
def populate_sensor_collection1(db_name, collection_name, base_path, mongo_uri="mongodb://localhost:27017/", chunk_size=10000):
    """
    Populate the MongoDB motionsensor collection with motion sensor data.
    ...
    Args:
        ...
        base_path (str): Path where the user folders are located.
        mongo_uri (str): MongoDB URI to connect to the database.
        chunk_size (int): Number of data points per chunk in the motion sensor data.
        ...
        if os.path.isdir(user_path):
            ...
            for recording_folder in os.listdir(user_path):
                ...
                if os.path.isdir(recording_path):
                    ...
                    # Process files in the recording folder
                    for file_name in os.listdir(recording_path):
                        if file_name in ["Hand_Motion.txt", "Hips_Motion.txt", "Torso_Motion.txt", "Bag_Motion.txt"]:
                            ...
                            # Generate motion sensor JSON data in chunks
                            for chunked_data in create_motion_sensor_json1(motion_file_path,recording_id,sensor_location, chunk_size):
                                try:
                                    start_time = time.time()
                                    collection.update_one({"_id": chunked_data["_id"]}, {"$set": chunked_data}, upsert=True)
                                    #collection.insert_many(chunked_data, ordered=False)
                                    end_time = time.time()
                                    insertion_times.append(end_time - start_time)
                                    print(f"Inserted/Updated document: {chunked_data['_id']} (Time: {end_time - start_time:.4f} seconds)")
                                except Exception as e:
                                    print(f"Error inserting document {chunked_data['_id']}: {e}")
                ...
                if os.path.isdir(recording_path):
                    for file_name in os.listdir(recording_path):
                        if file_name.endswith('_Motion.txt'):
                            ...
                            for i in range(0, len(motion_data["acceleration"]), chunk_size):
                                ...
                                # Append to the list of all_motion_sensors
                                all_motion_sensors.append(chunked_data)
                            
    return all_motion_sensors

def create_motion_sensor_json1(motion_file_path,recording_id,sensor_location, chunk_size=10000):
    """
    Generate chunks of motion sensor data structured as JSON documents.

    Args:
        motion_file_path (str): Path to the motion sensor file.
        chunk_size (int): Number of data points per chunk.

    Yields:
        dict: A JSON document containing a chunk of motion sensor data.
    """
    # Replace with actual parsing logic for the motion file
    motion_data = parse_motion_file(motion_file_path)  # Placeholder function

    # Extract metadata from the file path
    """ file_name = os.path.basename(motion_file_path)
    recording_id = file_name.split('_')[0]  # Adjust based on naming convention
    sensor_location = file_name.split('_')[0].lower() """

    # Chunk the data and yield JSON documents
    for i in range(0, len(motion_data["acceleration"]), chunk_size):
        yield {
            "_id": f"{recording_id}-{sensor_location}-{i}",
            "recording_id": recording_id,
            "sensor_location": sensor_location,
            "acceleration": motion_data["acceleration"][i:i + chunk_size],
            "gyroscope": motion_data["gyroscope"][i:i + chunk_size],
            "magnetometer": motion_data["magnetometer"][i:i + chunk_size],
            "orientation": motion_data["orientation"][i:i + chunk_size],
            "gravity": motion_data["gravity"][i:i + chunk_size],
            "linear_acceleration": motion_data["linear_acceleration"][i:i + chunk_size],
            "pressure": motion_data["pressure"][i:i + chunk_size],
            "altitude": motion_data["altitude"][i:i + chunk_size],
            "temperature": motion_data["temperature"][i:i + chunk_size]
        }
...
def create_wifi_json(base_path):
    ...
    """
    all_wifi = []
    max_chunk_size = 900  # Adjust this to a reasonable chunk size
...
def create_cells_json(base_path):
    ...
    max_chunk_size = 100  # Adjust this to a reasonable chunk size
        ...
        if os.path.isdir(user_path):  # Ensure it is a directory (user folder)
            ...
            for recording_folder in os.listdir(user_path):
                ...
                if os.path.isdir(recording_path):  # Ensure it is a directory (recording folder)
                    ...
                    for sensor_file in os.listdir(recording_path):
                        if (('Cells' in sensor_file) and ('Depr' not in sensor_file)):  # Identify Cells sensor file
                            ...
                            # Split wifi_data into chunks
                            for i in range(0, len(cells_data), max_chunk_size):
                                cells_chunk = cells_data[i:i + max_chunk_size]
                                
                                cells_json = {
                                    "_id": f"{user_folder}-{recording_folder}-{sensor_file.replace('.txt', '')}",
                                    "recording_id": f"{user_folder}-{recording_folder}",
                                    "sensor_location": sensor_location,
                                    "cells_data": cells_chunk
                                }
...
########################################################################################################################################## 
def create_label_sensor_json(base_path, chunk_size=1000):
    """
